...
def register():
    ...
    if request.method == 'POST':
        ...
        phone_number = request.form.get('phone', '').strip()  # optional
    ...
    # GET request - pass any form data for pre-fill (optional)
...
def login():
    ...
    if request.method == 'POST':
        ...
        if user and user.check_password(password):
            # Check if user has any account (migration safety)
...
def resend_verification():
    """Resend verification email — supports both AJAX and normal requests"""
    ...
    # Return JSON for AJAX requests (from the banner button in base_user.html)
    if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
